    ...
    <c:forEach var="status" items="${statusList}">
        <tr>
            ...
            <td><c:forEach var="image" items="${status.getStatusAttachmentList()}">
                <img src="/images/${image.getAttachmentPath()}"  height="50px" width="50px"  style="object-fit: cover" class="img-fluid border border-danger"/>
            </c:forEach></td>
        <%--                <td class="td">${status.getAttachment().getAttachmentPath()}</td>--%>
            <td><a class="btn btn-success" href="/status/update/${status.getId()}">Update</a></td>
